exports.up = async function (knex) {
  ...
  let carMakes = await knex("car_makes").select("id").select("builder_layers");
  let carMakesMap = {};
  for (let carMake of carMakes) {
    carMakesMap[carMake.id] = carMake;
  }
  ...
  for (let offset = 0; offset < totalLayers; offset += QUERY_LIMIT)
    queries.push(
      knex("builder_layers")
        ...
        .select("layer_data")
        .select("builder_schemes.car_make")
        .leftJoin(
          "builder_schemes",
            ...
            for (let j = 0; j < BATCH_SIZE; j++) {
              ...
              try {
                layersJSON = JSON.parse(
                  carMakesMap[originalData.car_make].builder_layers.toString(
                    "utf-8"
                  )
                );
              } catch (e) {
              ...
              if (layer_data.name) {
                ...
                if (!layer) {
                  ...
                  if ("#" + layer.color === layer_data.color) {
